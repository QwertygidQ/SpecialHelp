...
from .forms import (
    ...
    NewPasswordForm,
    CommentForm,
    UserPictureUpdateForm,
)
from .helpers import (
    unauthenticated_required,
    get_next_page,
    get_info_for_tag_and_validate,
)
...
@app.route("/")
def index():
    return render_template("index.html", title=gettext("SpecialHelp"))
...
def signin():
    ...
    return render_template("signin.html", title=gettext("Sign in"), form=form)
...
def signup():
    ...
    return render_template("signup.html", title=gettext("Registration"), form=form)
...
def edit_profile():
    ...
    if pictureform.picture_update_submit.data and pictureform.validate_on_submit():
        return_code = image_upload.save_photo(
            pictureform.picture.data, current_user
...
def reset_password():
    ...
    if form.validate_on_submit():
        ...
        if user is not None:
            ...
            flash(
                gettext(
                    "Message with a password reset link has been sent to your email"
                ),
                "message",
            )
    ...
    return render_template(
        "reset_password.html", title=gettext("Reset password"), form=form
    )
...
def reset_password_confirmed(token):
    ...
    return render_template(
        "reset_password_confirmed.html", title=gettext("New password"), form=form
    )
...
def business_page(business_link):
    business = Business.query.filter(
        func.lower(Business.link) == business_link.lower()
    ).first()
    if business is not None:
        form = None
        has_not_commented = current_user.is_authenticated and current_user not in [
            comment.author for comment in business.comments
        ]
        ...
        if has_not_commented:
            ...
            if form.validate_on_submit():
                ...
                comment = Comment(
                    rating=rating, text=text, business=business, author=current_user
                )
...
def tag_list_page(tag_name):
    if tag_name is None or Tag.query.filter_by(name=tag_name).first() is None:
        abort(404)
...
def get_businesses():
    err_json = jsonify(
        {"status": "error", "desc": gettext("Server got invalid data from the client.")}
    )

    if not request.json or not all(
        param in request.json for param in ["type", "page", "reverse"]
    ):
        return err_json
    ...
    if sort_type == "location":
        ...
        if (
            ...
            or type(max_dist) != int
        ):
            return err_json

        if (
            not -90.0 <= lat <= 90.0 or not -180.0 <= lon <= 180.0
        ):  # TODO: properly check float ranges?
            return err_json
        ...
        if reverse:
            ...
            )  # we want to normally show the highest rating first
    elif sort_type == "date":
        if reverse:
            query = query.order_by(Business.date_created)
        else:
            query = query.order_by(
                Business.date_created.desc()
            )  # we want to normally show the latest added businesses
    else:
        return err_json
    ...
    if not businesses:
        return jsonify(
            {"status": "error", "desc": gettext("No such organisations exist.")}
        )
    ...
    return_dict = {
        ...
        "businesses": [
            {
                "img": "/uploads/" + business.image.filename
                if business.image
