...
def send_reset_password_email(user):
    ...
    send_message(
        _("[SpecialHelp] Password reset"),
        app.config["MAIL_USERNAME"],
        [user.email],
        text_body=render_template("reset_password_email.txt", user=user, token=token),
        html_body=render_template("reset_password_email.html", user=user, token=token),
    )
