...
class User(db.Model, UserMixin):
    ...
    locale = db.Column(
        db.String(5), default="NONE", nullable=False
    )  # e.g. 'en-gb' or 'ru'
...
class Comment(db.Model):
    ...
    def __repr__(self):
        return "<Comment by {} on {}: {}>".format(
            self.author.username, self.business.name, self.text
        )
...
class Business(db.Model):  # company/event
    ...
    tags = db.relationship(
        "Tag", secondary=business_tag_table, backref="businesses", lazy="dynamic"
    )
    ...
    def recalculate_rating(self):
        if len(self.comments.all()) == 0:
            ...
            self.rating = int(
                sum(comment.rating for comment in self.comments)
    ...
    def calculate_dist_to_user(self, user_coords):
        ...
        a = (
            math.sin(rad_delta_lat / 2) ** 2
            + math.cos(rad_lat_usr)
            * math.cos(rad_lat_dest)
            * math.sin(rad_delta_lon / 2) ** 2
        )
    ...
    def calculate_dist_to_user(cls, user_coords):
        ...
        a = func.pow(func.sin(rad_delta_lat / 2), 2) + func.cos(rad_lat_usr) * func.cos(
            rad_lat_dest
        ) * func.pow(func.sin(rad_delta_lon / 2), 2)
